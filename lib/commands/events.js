...
function memory() {
  if (mem === null) {
    try {
      mem = { events: require('../memory/events') };
    } catch (e) {
      if (process.env.XHS_DEBUG) console.warn('[events] memory unavailable:', e.message);
      mem = false;
    }
  }
  return mem || null;
...
async function cmdEvents(ctx, args) {
  ...
  if (!m) {
    ...
    if (asJson) return { error: err.message };
  ...
  const rows = m.events.query(filters, { limit: Math.max(tail, 1), order: 'desc' }).reverse();
  ...
  for (const r of rows) {
    ...
    const tags = [];
    if (r.noteId) tags.push(`n=${r.noteId}`);
