...
async function cmdSearch(ctx, args) {
  ...
  const rawCount = getFlag(args, '--count', 20);
  // 小红书 API page_size 最小值为 20，低于此值会返回空 items
  const count = Math.max(rawCount, 20);
  ...
  const filtered = rawItems.filter(d => (d.model_type === 'note' || d.note_card)).map(parseSearchItem);
  // 如果用户请求的 count < 20，截取到请求数量
  const items = rawCount < 20 ? filtered.slice(0, rawCount) : filtered;
