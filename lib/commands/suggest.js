...
const DEFAULT_POST_INTERVAL_MS = 60000; // 每条评论间隔 60 秒，防止风控

/**
 * LLM 回复建议
 * @param {object} ctx - { bridge, audit, config, cmdAnalyze, cmdPost }
 * @param {string[]} args - [note_id, --auto, --min-priority N]
 */
async function cmdSuggest(ctx, args) {
  ...
  for (let i = 0; i < autoList.length; i++) {
    ...
    if (auto && s.reply) {
      // 非首条发布前等待间隔
      if (postedCount > 0) {
        console.error(`⏳ 等待 ${postInterval / 1000}s 后发布下一条... (${postedCount + 1}/${autoList.length})`);
        await new Promise(r => setTimeout(r, postInterval));
      }
      try {
        const postResult = await ctx.cmdPost([s.cid ? noteId : '', s.reply, '--reply-to', s.cid || '']);
