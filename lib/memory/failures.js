// lib/memory/failures.js — 发布失败模式
//
// 设计要点：
// - 每条 post 失败按"原因 signature"做归一化，UPSERT 累加 hit_count；
//   signature 是稳定字符串：例如 'xhs_code=461'、'risk_control'、'dup_text'。
// - mitigation 不在这里硬编码业务，由调用方记录或从 ENV/JSON 配置加载。
// - 目的：① 给 suggest 注入「避雷清单」；② 自适应风控冷却使用。
 ...
 * 输入可以是 Error / string / { code, msg } 等，返回稳定字符串。
 */
function classify(input) {
  if (input == null) return 'unknown';
  if (typeof input === 'object') {
    // xhs envelope: { code, msg, ... }
    if (input.code != null && input.code !== 0 && input.code !== '0') {
      return `xhs_code=${input.code}`;
    }
    if (input.signature) return String(input.signature);
    if (input.envelope && input.envelope.code != null) return `xhs_code=${input.envelope.code}`;
    if (input.message) return classify(input.message);
    return 'unknown';
  }
  const msg = String(input);
  // 抓 xhs[461 ...] 这类标签
  const m1 = msg.match(/xhs\[(-?\d+)\b/);
  if (m1) return `xhs_code=${m1[1]}`;
  // 油猴侧典型错误
  ...
  if (/RISK_CONTROL|风控|-10000/i.test(msg)) return 'risk_control';
  if (/NO_PERMISSION|461/i.test(msg)) return 'xhs_code=461';
  if (/NOT_LOGGED_IN|-100/i.test(msg)) return 'not_logged_in';
  if (/重复|duplicate|dup_text/i.test(msg)) return 'dup_text';
  if (/SIGNATURE_/i.test(msg)) return 'signature_invalid';
  // 兜底：截前 80 字符做 signature
...
function record(input, opts = {}) {
  try {
    ...
    if (process.env.XHS_DEBUG) console.warn('[failures.record] failed:', e.message);
    return false;
  }
}
